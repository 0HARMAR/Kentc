...
void AsmWriter::movMemToMem(const string& src, const string& dest, const string& spilledTransitReg, int bitWide)
{

	push(formatReg(spilledTransitReg, 64));
	mov(src, formatReg(spilledTransitReg, bitWide), bitWideToMovSubfix[bitWide]);
	mov(formatReg(spilledTransitReg, bitWide), dest, bitWideToMovSubfix[bitWide]);
	pop(formatReg(spilledTransitReg, 64));
}
...
void AsmWriter::pushfq()
{
	lines.push_back("	pushfq");
}

void AsmWriter::popfq()
{
	lines.push_back("	popfq");
}
