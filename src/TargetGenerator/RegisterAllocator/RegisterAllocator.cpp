//
// Created by hemin on 25-7-19.
//

#include "RegisterAllocator.h"
#include "../CallingConvention.h"

extern bool isMain;
RegisterAllocator::RegisterAllocator(AsmWriter& asmWriter, int& stackSize, map<string, int>& stackOffset, string& currentFunction) :
asmWriter(asmWriter), stackSize(stackSize), stackOffset(stackOffset) , currentFunction(currentFunction){}

// alloc register for immediate
string RegisterAllocator::allocReg(vector<string> exclude)
{
	string reg = freeRegs.back();
	auto it = find_if(freeRegs.begin(), freeRegs.end(), [&](const auto& r)
			{
				return find(exclude.begin(), exclude.end(), r) == exclude.end();
			});

	if (it != freeRegs.end())
	{
		reg = *it;
		freeRegs.erase(it);
	} else
	{
		reg = "";
	}
	if (reg != "")freeRegs.pop_back();
	return spillVar();
}

string RegisterAllocator::allocReg(string var, string type, vector<string> exclude)
{
	if (!freeRegs.empty())
	{
		string reg = freeRegs.back();
		// no exclude or not exclude register, use it directly
		if (exclude.empty() || find(exclude.begin(), exclude.end(), reg) == exclude.end())
		{
			freeRegs.pop_back();
		} else
		{
			auto it = find_if(freeRegs.begin(), freeRegs.end(), [&](const auto& r)
			{
				return find(exclude.begin(), exclude.end(), r) == exclude.end();
			});

			if (it != freeRegs.end())
			{
				reg = *it;
				freeRegs.erase(it);
			} else
			{
				reg = "";
			}
		}
		if (reg != "")
		{
			varToReg[var] = reg;
			allocaHistory.push_back("+ " + var + ": " + reg);
			return reg;
		}
	}
	return spillVar(var, type);
}

string RegisterAllocator::allocaArguments(string var, int index)
{
	vector<string> argumentRegs = CallingConvention::parameters;
	string targetReg = "%" + argumentRegs[index];
	varToArgumentReg[var] = targetReg;
	return targetReg;
}


// if free register null, put temp var to stack
string RegisterAllocator::spillVar(string var, string type)
{
	int typeSize = variableMap[type];
	int spillSlot = stackOffset[currentFunction] - typeSize;
	int allocaSize = typeSize;
	// check 8 bytes align
	if (typeSize == 8)
	{
		int oldSlot = spillSlot;
		spillSlot &= ~0x7;
		allocaSize = -(spillSlot - oldSlot) + typeSize;
	}
	if (spillSlot < stackSize) extandStack(isMain);
	stackOffset[currentFunction] -= allocaSize;
	spilledVars[var] = spillSlot;

	allocaHistory.push_back("+ " + var + ": " + std::to_string(stackOffset[currentFunction]) + "(%rbp)");
	return std::to_string(stackOffset[currentFunction]) + "(%rbp)";
}

// if free register null, put immediate to stack
string RegisterAllocator::spillVar()
{
	int spillSlot = stackOffset[currentFunction] - 4;
	if (spillSlot < stackSize) extandStack(isMain);
	stackOffset[currentFunction] -= 4;

	return std::to_string(stackOffset[currentFunction]) + "(%rbp)";
}
// choose first var
string RegisterAllocator::selectSpillCandidate()
{
	for (auto& pair : varToReg)
	{
		if (find(CallingConvention::callerSaved.begin(), CallingConvention::callerSaved.end(),
			pair.second) != CallingConvention::callerSaved.end())
		{
			return pair.first;
		}
	}
	return varToReg.begin()->first;
}

string RegisterAllocator::getTempVarLocation(string operand)
{
	if (operand[0] == '%')
	{
		if (varToReg.count(operand)) return varToReg[operand];
		if (spilledVars.count(operand)) return to_string(spilledVars[operand]) + "(%rbp)";
		if (varToArgumentReg.count(operand)) return varToArgumentReg[operand];
	}
	return "";
}

void RegisterAllocator::handleCall()
{
	for (auto& pair : varToReg)
	{
		if (find(CallingConvention::callerSaved.begin(), CallingConvention::callerSaved.end(),
			pair.second) != CallingConvention::callerSaved.end())
		{
			// TODO generate asm code to save the register

			spilledVars[pair.first] = stackOffset[currentFunction] - 4;
			stackOffset[currentFunction] -= 4;
		}
	}
}

void RegisterAllocator::freeReg(string var)
{
	if (varToReg.count(var))
	{
		string reg = varToReg[var];
		freeRegs.push_back(reg);
		varToReg.erase(var);
		allocaHistory.push_back("- " + var + ": " + reg);
	}
}

string RegisterAllocator::getVarInRegister(string reg)
{
	for (const auto& pair : varToReg)
	{
		if (pair.second == reg) return pair.first;
	}
	return "";
}

// if var bind a reg, cancel the bind, and alloca a new reg for it,
// return the new reg
string RegisterAllocator::spillRegister(string var, string type)
{
	vector<string> exclude;
	if (varToReg.count(var))
	{
		string reg = varToReg[var];
		freeRegs.push_back(reg);
		varToReg.erase(var);
		exclude.push_back(reg);
		allocaHistory.push_back("- " + var + ": " + reg);
	}

	return allocReg(var, type, exclude);
}
// binding a var to reg, if var is binding, return old reg
string RegisterAllocator::allocFixReg(string var, string reg)
{
	if (varToReg.count(var))
	{
		string old_reg = getTempVarLocation(var);
		// free var binding reg
		freeReg(var);

		// remove reg form freeRegs
		freeRegs.erase(std::remove(freeRegs.begin(), freeRegs.end(), reg), freeRegs.end());

		varToReg[var] = reg;
		allocaHistory.push_back("- " + var + ": " + reg);
		return old_reg;

	} else
	{
		// remove reg form freeRegs
		freeRegs.erase(remove(freeRegs.begin(), freeRegs.end(), reg), freeRegs.end());

		varToReg[var] = reg;
		allocaHistory.push_back("- " + var + ": " + reg);
		return "";
	}
}

void RegisterAllocator::extandStack(bool inMain)
{
	// TODO calculate need stack size
	stackSize -= 16;
	asmWriter.pushfq();
	asmWriter.push("%rax");
	asmWriter.mov("8(%rsp)", "%rax", "q");
	asmWriter.push("%rax");
	asmWriter.popfq();
	asmWriter.mov("(%rsp)", "%rax", "q");
	if (inMain);
	else
	{
		asmWriter.push("%rax");
		// 24(%rsp) is the old return address slot
		asmWriter.mov("24(%rsp)", "%rax", "q");
		// -8(%rsp) is the new return address slot
		asmWriter.mov("%rax", "8(%rsp)", "q");
		asmWriter.pop("%rax");
	}
}

bool RegisterAllocator::haveFreeReg()
{
	if (freeRegs.size() == 0) return false;
	return true;
}

string RegisterAllocator::getRandomTransitReg(string exclude)
{
	static vector<string> Regs = {"%rdi", "%rsi", "%rax", "%rbx", "%rcx", "%rdx",
		"%r8","%r9", "%r10", "%r11", "%r12", "%r13", "%r14", "%r15"};
	static string lastReg;

	static bool seedInitialized = false;
	if (!seedInitialized)
	{
		srand(time(nullptr));
		seedInitialized = true;
	}

	string newReg;
	int attempts = 0;
	const int MAX_ATTEMPTS = 1000;

	do
	{
		string key = "harmar";
		int salt = rand();
		string salted = key + to_string(salt);
		size_t index = hash<string>{}(salted) % Regs.size();
		newReg = Regs[index];

		bool excludeCondition = newReg == exclude;
		bool lastRegCondition = (!lastReg.empty() && newReg == lastReg);

		if (!excludeCondition && (lastReg.empty() || !lastRegCondition
			|| Regs.size() == 1)) break;
		attempts++;
	} while (attempts < MAX_ATTEMPTS);

	if (attempts >= MAX_ATTEMPTS)
	{
		for (const auto& reg : Regs)
		{
			if (reg != exclude && (lastReg.empty() || reg != lastReg))
			{
				newReg = reg;
				break;
			}
		}
	}

	lastReg = newReg;
	return newReg;

}

